...
#  将原始数据word转换成index
def load_data(traning_data_path, vocab_word2index, vocab_label2index, sentence_len, name_scope, training_portion=0.95,
              tokenize_style='char'):
    """
    ...
    for i, row in enumerate(spamreader):  ##row:['\ufeff1', '\ufeff怎么更改花呗手机号码', '我的花呗是以前的手机号码，怎么更改成现在的支付宝的号码手机号', '1']
        ...
        features_vector = data_mining_features(i, row[1], row[2], vocab_word2index, word_vec_fasttext_dict,
                                               word_vec_word2vec_dict, tfidf_dict, n_gram=8)
    ...
    X1_final, X2_final, BLUE_SCORE_final, Y_final, training_number_big = get_training_data(X1[0:training_number],
    ...
    valid = (
    X1[training_number + 1:valid_end], X2[training_number + 1:valid_end], BLUE_SCORES[training_number + 1:valid_end],
...
def create_vocabulary(training_data_path, vocab_size, name_scope='cnn', tokenize_style='char'):
    ...
    if os.path.exists(cache_path):
        ...
        for i, row in enumerate(
                spamreader):  # row:['\ufeff1', '\ufeff怎么更改花呗手机号码', '我的花呗是以前的手机号码，怎么更改成现在的支付宝的号码手机号', '1']
...
# 数据特征工程
def data_mining_features(index, input_string_x1, input_string_x2, vocab_word2index, word_vec_fasttext_dict,
                         word_vec_word2vec_dict, tfidf_dict, n_gram=8):
    """
    得到数据语义特征从两个句子中
